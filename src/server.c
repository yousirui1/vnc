    ...
    if(read_msg_order(req->head_buf) == READY_MSG)
    {
        ...
        if(code == 200)
        {
            ...
            *(int *)&tmp[0] = req->display_id;
			...
			//DEBUG("pipe SER_PLAY_MSG %d control_play_flag %d +++++++++++", *(int *)&tmp[0], control_display->play_flag);
			//DEBUG("displays[display_size].play_flag %d",displays[display_size].play_flag);
            send_pipe(buf, SER_PLAY_MSG, sizeof(int));
			req->status = DONE;
            return SUCCESS;
        }
...
static int recv_options(rfb_request *req)
{
	...
	DEBUG("read_msg_order(req->head_buf)  %d OPTIONS_MSG_RET %d", read_msg_order(req->head_buf), OPTIONS_MSG_RET);
	DEBUG("options error");
    return ERROR;
}
...
int start_control(int id)
{
	...
	//control_display.
	//displays[id].req->status = OPTIONS;
	//memcpy(&control_display.req, &displays[id].req,  sizeof(rfb_request));
	send_options(control_display->req);	
	DEBUG("-------------- control_display->req->fd %d -------------------", control_display->req->fd);
}
